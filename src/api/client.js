...
export class Client {
  ...
  call = async (url: string, params: SpecialParameters = {}, fetchParameters: FetchParameters = {}) => {
    let finalUrl = url;

    // add explicitely specified parameters
    if (params.perPage) {
      finalUrl = `${finalUrl}${finalUrl.includes('?') ? '&' : '?'}per_page=${Number(params.perPage).toString()}`;
    }
    ...
    const parameters: any = {
      method,
      headers: {
        'User-Agent': `RepoGithub/${version} ${Platform.OS}`,
  ...
  get = ({ fetchParameters, ...config }: CallParameters): CallType => ({
    ...
    params: {},
    ...config,
    fetchParameters: merge({ method: this.Method.GET, headers: { Accept: this.Accept.JSON } }, fetchParameters),
  });
  ...
  delete = ({ fetchParameters, ...config }: CallParameters): CallType => ({
    ...
    params: {},
    ...config,
    fetchParameters: merge({ method: this.Method.DELETE, headers: { Accept: this.Accept.JSON } }, fetchParameters),
  });
  ...
  create = ({ fetchParameters, ...config }: CallParameters): CallType => ({
    ...
    params: {},
    ...config,
    fetchParameters: merge({ method: this.Method.POST, headers: { Accept: this.Accept.JSON } }, fetchParameters),
  });

  query = ({ fetchParameters, query, variables, ...config }: GraphQLCallParameters): CallType => ({
    type: 'query',
    endpoint: 'graphql',
    params: {},
    ...config,
  ...
  activity = {
     ...
     * @param {string} userId
     */
    getEventsReceived: (userId: string, params: SpecialParameters = {}): CallType =>
      this.list({
