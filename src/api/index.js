...
export const v3 = {
  ...
  parameters: (accessToken, method = METHOD.GET, accept = ACCEPT.JSON, body = {}) => {
  ...
  count: async (url, accessToken) => {
    const finalUrl = url.indexOf('?') !== -1 ? `${url}&per_page=1` : `${url}?per_page=1`;
  ...
  getDiff: async (url, accessToken) => {
    const response = await v3.call(url, v3.parameters(accessToken, METHOD.GET, ACCEPT.DIFF));

    return response.text();
  },
  getHtml: async (url, accessToken) => {
    const response = await v3.call(url, v3.parameters(accessToken, METHOD.GET, ACCEPT.HTML));

    return response.text();
  },
  getFull: async (url, accessToken) => {
    const response = await v3.call(url, v3.parameters(accessToken, METHOD.GET, ACCEPT.FULL));
  ...
  head: async (url, accessToken) => {
    const response = await v3.call(url, v3.parameters(accessToken, METHOD.HEAD));

    return response;
  },
  patch: async (url, accessToken, body = {}) => {
    const response = await v3.call(url, v3.parameters(accessToken, METHOD.PATCH, ACCEPT.JSON, body));

    return response;
  },
  patchFull: async (url, accessToken, body = {}) => {
    const response = await v3.call(url, v3.parameters(accessToken, METHOD.PATCH, ACCEPT.FULL, body));
  ...
  postHtml: async (url, accessToken, body = {}) => {
    const response = await v3.call(url, v3.parameters(accessToken, METHOD.POST, ACCEPT.HTML, body));

    return response.text();
  },
  postFull: async (url, accessToken, body = {}) => {
    const response = await v3.call(url, v3.parameters(accessToken, METHOD.POST, ACCEPT.FULL, body));
  ...
  put: async (url, accessToken, body = {}) => {
    const response = await v3.call(url, v3.parameters(accessToken, METHOD.PUT, ACCEPT.JSON, body));
...
export const fetchAuthUserOrgs = accessToken => v3.getJson('/user/orgs', accessToken);

export const fetchUser = (user, accessToken) => v3.getJson(`/users/${user}`, accessToken);

export const fetchUserOrgs = (user, accessToken) => v3.getJson(`/users/${user}/orgs`, accessToken);

export const fetchReadMe = (user, repository, accessToken) =>
  v3.getHtml(`/repos/${user}/${repository}/readme?ref=master`, accessToken);

export const fetchOrg = (orgName, accessToken) => v3.getJson(`/orgs/${orgName}`, accessToken);

export const fetchOrgMembers = (orgName, accessToken) => v3.getJson(`/orgs/${orgName}/members`, accessToken);

export const fetchPostIssueComment = (body, owner, repoName, issueNum, accessToken) =>
  v3.postFull(`/repos/${owner}/${repoName}/issues/${issueNum}/comments`, accessToken, { body });

export const fetchDeleteIssueComment = (issueCommentId, owner, repoName, accessToken) =>
  v3.delete(`/repos/${owner}/${repoName}/issues/comments/${issueCommentId}`, accessToken);
...
export const fetchEditIssueComment = (
  ...
  editParams: any,
  accessToken: string
) => v3.patchFull(`/repos/${owner}/${repoName}/issues/comments/${issueCommentId}`, accessToken, editParams);

export const fetchEditIssue = (owner, repoName, issueNum, editParams, accessToken) =>
  v3.patchFull(`/repos/${owner}/${repoName}/issues/${issueNum}`, accessToken, editParams);

export const fetchChangeIssueLockStatus = (owner, repoName, issueNum, currentStatus, accessToken) =>
  v3[currentStatus ? 'delete' : 'put'](`/repos/${owner}/${repoName}/issues/${issueNum}/lock`, accessToken);
...
export const fetchMarkAllNotificationsAsRead = accessToken => v3.put('/notifications', accessToken);

export const fetchStarCount = (owner, accessToken) => v3.count(`/users/${owner}/starred`, accessToken);
...
export const fetchMergePullRequest = (repo, issueNum, commitTitle, commitMessage, mergeMethod, accessToken) =>
...
export const fetchSubmitNewIssue = (owner, repo, issueTitle, issueComment, accessToken) =>
...
export async function fetchAccessToken(code, state) {
  ...
  const response = await fetch(GITHUB_OAUTH_ENDPOINT, authParameters(code, state));

  return response.json();
}

export const fetchNotificationsCount = accessToken => v3.count('/notifications?per_page=1', accessToken);
