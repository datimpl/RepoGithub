...
import { fetchAccessToken, fetchAuthUser, fetchAuthUserOrgs, fetchUserOrgs, fetchStarCount } from '../../api';
import { LOGIN, LOGOUT, GET_AUTH_USER, GET_AUTH_ORGS, CHANGE_LOCALE, GET_AUTH_STAR_COUNT } from './auth.type';
...
export const getOrgs = () => {
  return (dispatch, getState) => {
    ...
    Promise.all([fetchAuthUserOrgs(accessToken), fetchUserOrgs(login, accessToken)])
      .then(data => {
        const orgs = data[0].concat(data[1]);

        dispatch({
          type: GET_AUTH_ORGS.SUCCESS,
          payload: uniqBy(orgs, 'login').sort((org1, org2) => org1.login > org2.login),
        });
      })
      .catch(error => {
        dispatch({
